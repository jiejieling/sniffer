...
char *getcurtime(void)
{
        ...
        ptr = malloc(17 + 11);
        if (localtime_r(&time.tv_sec, &lt) == NULL)
        {
                perror("localtime_r");
                return NULL;
        }
        //2012-01-01 11:11
        if (strftime(ptr, 17, "%Y-%m-%d %H:%M", &lt) == 0){
                perror("strftime");
                return NULL;
        }
        //2012-01-00
        if (strftime(&ptr[17], 11, "%Y-%m-00", &lt) == 0){
                perror("strftime");
                return NULL;
        }
        return ptr;
...
void work(void)
{
	...
	mysql_init(&my_conn);
	if ( ! mysql_real_connect(&my_conn, MY_HOST, MY_USER, MY_PASS, MY_DB, 3306, MY_SOCK,0))
	{
